 ...
 * @diva_info: this is the struct address
 *
 * Return: 1 if it's on interactive mode, 0 if otherwise
 */
int diva_shell_interactive(info_t *diva_info)
{
	return (isatty(STDIN_FILENO) && diva_info->readfd <= 2);
}

/**
 * is_diva_delimiter - a function that checks if character is a delimiter
 * @diva_c: this is the char to check
 * @diva_delim: this is the delimiter string
...
/**
 * diva_str_to_int - function that converts a string to an integer
 * @diva_str: this is the string to be converted
 * Return: 0 if no numbers in string, converted number otherwise
 */

int diva_str_to_int(char *diva_str)
{
	int diva_i, diva_sign = 1, diva_flag = 0, diva_result = 0;

	for (diva_i = 0; diva_str[diva_i] != '\0' && diva_flag != 2; diva_i++)
	{
		if (diva_str[diva_i] == '-')
			diva_sign *= -1;

		if (diva_str[diva_i] >= '0' && diva_str[diva_i] <= '9')
		{
			diva_flag = 1;
			diva_result *= 10;
			diva_result += (diva_str[diva_i] - '0');
		}
		else if (diva_flag == 1)
			diva_flag = 2;
	}
